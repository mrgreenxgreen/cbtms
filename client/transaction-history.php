...
if(isset($_SESSION['sessionId'])){
    ...
    $pending_result = mysqli_query($mysqli, "SELECT * FROM requests WHERE request_by = '$request_by' AND (request_status = '$request_status_completed'OR request_status = '$request_status_cancelled' ) AND isActive=1");
...
<h1><i class="fa-solid fa-book-open"></i> History </h1>
<div style="color:white;"> <h1>`</h1></div>
...
// QUERY FOR PENDING TRANSACTIONS
    while($row = mysqli_fetch_array($pending_result)){
        ...
        {
             echo       '<td style=\"color:red;\">'.$row['request_status'].'</td>';

        } elseif($row['request_status'] == 'COMPLETED')
        {
             echo       '<td style=\"color:green;\">'.$row['request_status'].'</td>';
